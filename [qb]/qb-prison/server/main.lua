...
RegisterNetEvent('prison:server:GiveJailItems', function(escaped)
    ...
    for _, v in pairs(Player.PlayerData.metadata['jailitems']) do
        exports['ps-inventory']:AddItem(src, v.name, v.amount, false, v.info, 'prison:server:GiveJailItems')
    end
    Player.SetMetaData('jailitems', {})
end)
...
RegisterNetEvent('prison:server:CheckChance', function()
    ...
    if not exports['ps-inventory']:AddItem(src, 'phone', 1, false, false, 'prison:server:CheckChance') then return end
    TriggerClientEvent('ps-inventory:client:ItemBox', src, sharedItems['phone'], 'add')
