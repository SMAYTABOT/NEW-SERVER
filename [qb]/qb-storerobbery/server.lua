...
RegisterNetEvent('qb-storerobbery:server:takeMoney', function(register, isDone)
    ...
    if isDone then
        ...
        exports['ps-inventory']:AddItem(src, 'markedbills', bags, false, info, 'qb-storerobbery:server:takeMoney')
        ...
        if math.random(1, 100) <= Config.stickyNoteChance then
            ...
            exports['ps-inventory']:AddItem(src, 'stickynote', 1, false, info, 'qb-storerobbery:server:takeMoney')
            TriggerClientEvent('ps-inventory:client:ItemBox', src, sharedItems['stickynote'], 'add')
        end
    end
end)
...
RegisterNetEvent('qb-storerobbery:server:SafeReward', function(safe)
    ...
    exports['ps-inventory']:AddItem(src, 'markedbills', bags, false, info, 'qb-storerobbery:server:SafeReward')
    TriggerClientEvent('ps-inventory:client:ItemBox', src, sharedItems['markedbills'], 'add')
    local luck = math.random(1, 100)
    local odd = math.random(1, 100)
    if luck <= 10 then
        exports['ps-inventory']:AddItem(src, 'rolex', math.random(3, 7), false, false, 'qb-storerobbery:server:SafeReward')
        TriggerClientEvent('ps-inventory:client:ItemBox', src, sharedItems['rolex'], 'add')
        if luck == odd then
            Wait(500)
            exports['ps-inventory']:AddItem(src, 'goldbar', 1, false, false, 'qb-storerobbery:server:SafeReward')
            TriggerClientEvent('ps-inventory:client:ItemBox', src, sharedItems['goldbar'], 'add')
        end
    end
end)
...
RegisterNetEvent('qb-storerobbery:server:removeAdvancedLockpick', function()
    ...
    if not Player then return end
    exports['ps-inventory']:RemoveItem(source, 'advancedlockpick', 1, false, 'qb-storerobbery:server:removeAdvancedLockpick')
end)

RegisterNetEvent('qb-storerobbery:server:removeLockpick', function()
    local Player = exports['qb-core']:GetPlayer(source)
    if not Player then return end
    exports['ps-inventory']:RemoveItem(source, 'lockpick', 1, false, 'qb-storerobbery:server:removeLockpick')
end)
